...
export default function OrderScreen() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      console.log('[ORDERS] Fetching active orders for user:', user.uid);
      
      // Try to load from AsyncStorage first (instant)
      try {
        // Try active orders cache first
        const activeCacheKey = `orders_active_${user.uid}`;
        const cachedOrders = await AsyncStorage.getItem(activeCacheKey);
        
        if (cachedOrders) {
          const parsed = JSON.parse(cachedOrders);
          console.log('[ORDERS] Loaded', parsed.length, 'active orders from cache');
          setOrders(parsed);
        } else {
          ...
          if (allOrdersCache) {
            ...
            console.log('[ORDERS] Loaded', activeOrders.length, 'active orders from all cache (filtered)');
            setOrders(activeOrders);
          }
        }
      } catch (cacheErr) {
        console.warn('[ORDERS] Cache load failed:', cacheErr.message);
      }
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          // Filter to show only active/preparing orders
          ...
          const sortedOrders = activeOrders.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
          console.log('[ORDERS] ✅ Found', sortedOrders.length, 'active orders from Firebase');
          setOrders(sortedOrders);
          
          // Cache active orders
          await AsyncStorage.setItem(`orders_active_${user.uid}`, JSON.stringify(sortedOrders));
        } else {
          console.log('[ORDERS] No orders found in Firebase');
        }
      } catch (firebaseErr) {
        // Silently fail and use cached data - no warnings shown to user
        if (firebaseErr.message !== 'Firebase timeout') {
          console.warn('[ORDERS] Firebase error:', firebaseErr.message);
        }
        ...
        { text: 'Cancel', style: 'cancel', onPress: () => console.log('[ORDER RECEIVED] Cancelled') },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              await AsyncStorage.setItem(`orders_active_${user.uid}`, JSON.stringify(updatedOrders));
              
              // Update completed orders cache
              const completedCache = await AsyncStorage.getItem(`orders_completed_${user.uid}`);
              const completedOrders = completedCache ? JSON.parse(completedCache) : [];
              completedOrders.unshift(updatedOrder);
  ...
  const renderOrderItem = ({ item }) => {
    ...
    // All active orders show as "Preparing"
    const statusStyle = { color: '#FF9800', icon: '👨‍🍳', label: 'PREPARING' };
  ...
  const emptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>You have no orders in progress. Start ordering now!</Text>
    </View>
  );
